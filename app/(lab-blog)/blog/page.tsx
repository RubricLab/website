...
import { type BlogCategory, blogpostCardFragment } from '@/lib/basehub/fragments/blog'
		...
		<>
			...
			>
				{async ([
					...
					// biome-ignore lint/suspicious/noExplicitAny: <explanation>
					{ _componentInstances: { blogPost: any }; site: { blog: any } }
				]) => {
					'use server'
					const { posts } = blog
					...
					const availableCategories = posts.items.reduce(
						// biome-ignore lint/suspicious/noExplicitAny: <explanation>
						(acc: BlogCategory[], post: { categories: any }) => {
					...
					return (
						<>
							...
							<div className="relative grid max-h-fold grid-cols-12">
								...
								<span className="col-span-1 hidden h-full bg-lines 2xl:block" />
