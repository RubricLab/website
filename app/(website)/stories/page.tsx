...
const StoryCard = ({ story: { title, summary, slug, imageUrl } }: StoryCardProps) => (
	<Link
		className="group hover:!opacity-100 relative w-full rounded-xl border bg-white p-10 opacity-90 shadow-2xl transition-opacity"
		href={`/stories/${slug}`}
		target="_blank"
	>
		<div className="relative mb-5 h-40 rounded-lg">
			<Image alt="Customer story image" className="rounded-md object-cover" fill src={imageUrl} />
		</div>
	...
	return (
		<div className="my-28 flex h-full flex-col gap-10 2xl:justify-center">
			...
			<div className="mx-auto flex max-w-xl flex-col items-center gap-5 py-10">
				...
				<Button body="Ready to tell a new story?" className="mt-32" href="/contact" variant="light" />
			</div>
		</div>
	)
}
