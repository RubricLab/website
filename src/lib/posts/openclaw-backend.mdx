...
To make onboarding as frictionless as possible, users only had to text a number: [8677-RUBRIC](sms:+18677782742?body=hi!).

![Text us (or scan a QR code) to get an OpenClaw.](/images/qr.jpeg "Text us (or scan a QR code) to get an OpenClaw.")
...
1. **A computer terminal is the everything tool.** From the shell, an agent can do just about anything on the internet, like write code to [order coffee](https://www.terminal.shop) then Slack you with the delivery time.
2. **A chat client is all you need.** This doesn’t have to be a fancy iOS app. Slack, SMS, Discord, a CLI - anything works so long as it supports (at a minimum) text in and notifications out. The medium does influence the messaging; for instance, we found SMS lent itself to short, simple requests.
3. **Being always-on is a must.** Using a computer is useful, but being able to save files and run long-running code supports use cases like serving a web app, setting reminders, or tracking a price.
...
Once [unblocked](https://rubriclabs.com/blog/unblocking-agents), the agent can also migrate its own tech extremely flexibly, e.g. from Python to Rust, from a closed-source to an open-source LLM, or from a VM in Singapore to the Raspberry Pi in your palm. In our in-person experiment, the underlying agent went from OpenClaw to [Hermes Agent](https://hermes-agent.nousresearch.com) to a custom, lightweight harness. Since the [agent is increasingly its memories](https://x.com/hwchase17/status/2042978500567609738), little changed for users.
...
[Coding agents](https://rubriclabs.com/blog/how-does-claude-code-actually-work) have [matured](https://epoch.ai/benchmarks?view=graph&tab=benchmarks&benchmark-selection=%7ESWE-Bench+verified%7Eterminalbench) to a point where they can routinely build working software. This unlocks a fundamentally new way to interface with computers, of which OpenClaw is one example.
...
Thank you to [Jihad Esmail](https://x.com/jaesmail), [Max Musing](https://x.com/MaxMusing), and [Justin Bellavance](https://www.justinbellavance.com) for their thoughtful input.
