...
Over the course of the night, several problems arose:
...
New technologies enable new form factors. Pioneers of the web [think](https://www.latent.space/p/pmarca?selection=0b21cbcc-1fd1-41b7-a76a-388c7cde642e) this is a big one. We think OpenClaw is exciting because it gets several UX assumptions deeply right:
...
2. **Being always-on is critical.** Using a computer is useful, but being able to save files and run long-running code supports [use cases](/blog/openclaw-backend#secret-codes) like serving a web app, setting reminders, or tracking a price.
...
We can push the standard OpenClaw setup further by giving it access to [rare digital goods](https://rubriclabs.com/blog/unblocking-agents), and by considering the system more as an admin dashboard than as a personal assistant.

Once unblocked, one more interesting [primitive](https://rubriclabs.com/blog/primitives-over-pipelines) emerges: the agent can migrate its own tech extremely flexibly, e.g. from OpenClaw to Hermes Agent, from a closed-source to an open-source LLM, from Python to Rust, or from a VM in Singapore to the Raspberry Pi in your palm.
...
Some things are harder to migrate. We found that the medium of SMS lends itself to simple, casual requests, which ran counter to our goal of getting people deploying sites within minutes. More on this in a future post.
...
If you’re an OpenClaw user and want to try deploying OpenClaws for your non-technical friends and family, here’s the kernel:

```md
---
name: subclaw
description: Spin up a sandboxed sub-OpenClaw / Hermes-Agent on its own public subdomain, addressable from a chat client (e.g. SMS via Twilio). For giving a non-technical user their own always-on coding agent without exposing your inference key.
when_to_use: You run an OpenClaw / Hermes-Agent for yourself and want to provision an isolated one for someone else.
---
