...
import { OpenclawSystemFigure } from '~/ui/blog/openclaw-backend'
...
<Figure id="openclaw-system">
  <OpenclawSystemFigure />
  <Figure.Caption>One supervisor OpenClaw, a fleet of per-user OpenClaws, and a single chat channel. The supervisor patches the harness on the spot; clients keep messaging their own agents.</Figure.Caption>
</Figure>

These coding sessions took several minutes each, but since the superadmin OpenClaw lived on the production server, we [skipped GitHub](https://mitchellh.com/writing/ghostty-leaving-github) entirely and builds took mere seconds.
...
We'd like to share a special thank you with [Jihad Esmail](https://x.com/jaesmail) and [Max Musing](https://x.com/MaxMusing) for their thoughtful feedback.
