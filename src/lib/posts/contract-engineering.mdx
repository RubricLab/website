...
Contracts differ from test-driven development in that they are durable and survive re-writes. As code changes, tests must be re-written, whereas contracts are expected to remain untouched except by the designer.
...
**Ambiguity.** Contracts currently follow two principles: being verifiable (or, at least, having a feedback loop to hill-climb), and being unambiguous enough that a junior developer could implement them. Formalizing and streamlining these guidelines into a reusable framework is non-trivial.
...
We’d like to share a special thank you with Jihad Esmail, Max Musing, and Erik Kaunismäki for their thoughtful pushback.