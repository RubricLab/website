...
Before we go deeper, it helps to understand what kind of system Claude Code is and where it sits in the landscape.

We said above that Claude Code is a coding agent that runs in your terminal. The important bit is that it can operate on your actual project by cycling through gather → act → verify, because it runs inside an agent harness.

### What is an agent harness? 
An *agent harness* is the layer around a language model that lets it use tools safely, keep state, and take real actions.

Think of it as the bridge between the model and an environment. In Claude Code, that environment is your filesystem, terminal commands, and test suite.

### Where does it sit?

One simple way to bucket coding tools is by how much of the agentic loop they can own:

- **Autocomplete**: suggests the next lines
- **Copilot**: helps you edit, but you still run and verify on your own
- **Agent**: can run the loop against a real project. Autonomy is a slider, and most tools move along it incrementally as permissions and verification improve.

That maps nicely to what Andrej Karpathy calls an [autonomy slider](https://youtu.be/LCEmiRjPEtQ?si=mMbOGKFrRf_esI2S&t=1228): the industry has moved from autocomplete, to chat copilots, to longer-running agents that can take on more of the loop.

Where Claude Code stands out is that it defaults near the autonomous agent end: terminal-native, harnessed, and opinionated about verification (tests, diffs, builds) with explicit permissions for risky actions.
...
MCP interacts with the context window in an unintuitive way: tool definitions usually hurt before tool results. If you dump hundreds of tool schemas into the prompt, you spend your budget on plumbing instead of reasoning.
