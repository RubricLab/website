...
The loop is the core mental model, but the loop alone is not enough. To operate on a real project, Claude Code runs inside an **agent harness** that gives the model tools, state, and permission boundaries.

This matters because most of the practical value comes from the harness layer, not just the base model.
...
Think of it as the bridge between model intent and real execution. In Claude Code, that execution environment is your filesystem, terminal commands, and test/build tooling.
...
Claude Code sits near the semi-autonomous end. It is terminal-native, strongly harnessed, and opinionated about verification (tests, diffs, builds), while still asking for explicit approval on risky actions.

So what does this architecture look like for a single request in practice? 
...
When you ask Claude Code to do real work, it’s mostly using a small, sharp set of tools. Anthropic publishes the canonical list in the [Claude Code docs](https://code.claude.com/docs/en/settings#tools-available-to-claude).
...
- **Interaction tool**: `AskUserQuestion` is how Claude gets unstuck when a required detail is missing by asking a set of multiple-choice questions to the user.
...
A `Task` is how Claude Code spins up a **sub-agent**. It is a separate, focused agent instance with its own context window that can do one scoped unit of work and return a condensed result to the main agent.

In practice, this is how Claude Code does deeper work without bloating the main thread. For example, the main agent can delegate “find all references to X” or “scan for failing tests and summarize” while keeping the primary narrative clean.
...
The most important part of Claude Code is the shape of the interfaces. The model decides what to do next and proposes a small, well-scoped action. The tool layer makes that action safe by validating inputs, enforcing constraints, and returning errors when an action is ambiguous.
