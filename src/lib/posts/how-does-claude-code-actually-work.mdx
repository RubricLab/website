...
- **`Bash`**: commands run in your environment. That makes verification concrete (run tests, check diffs), not rhetorical.
- **Web tools**: `WebSearch` is great for freshness and citations, while `WebFetch` pulls full pages. Both can be slow and token-heavy, so you want tight queries and bounded outputs.

The punchline is that agent quality is mostly interface design. The model proposes a small, well-scoped action, and the tool layer makes that action safe by enforcing constraints and returning errors when the action would be ambiguous.
...
Claude Code feels powerful because it is a tight system, and the biggest reason is the **environment** it lives in. Running in your terminal means it can reuse the primitives you already trust: files, diffs, tests, scripts, and your existing build tooling.

That is the practical takeaway: agent quality is mostly systems design. If you want an agent to be reliable, give it good primitives, small tool interfaces, clear permission boundaries, and a loop that makes verification cheap.

The second takeaway is that no task is too difficult if you can keep the loop moving. Break hard work into parallel sub-tasks, spin up sub-agents via `Task`, pull back condensed results, and use context compaction to keep the main thread focused as the work scales.

Zooming out, expect the autonomy slider to keep climbing. Agents will run longer, coordinate more parallel work, and take on larger end-to-end slices of projects as tool ecosystems mature and verification and permissions become more robust.
