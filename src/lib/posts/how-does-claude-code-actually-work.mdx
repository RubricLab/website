...
Claude Code is **closed source**, so we can’t inspect its internals directly. Instead, we have to infer the system from its **public surface area**, mainly the SDK’s tool catalog and docs.
...
- **Gather context** (read): It gathers context for e.g. read files, search the directory, and pull in terminal output to build a working set.
...
3. **Execution**: tools run in the **CLI environment** (read files, search, run commands) and return results.
...
At the center of Claude Code is… Claude. The model reads your project, keeps a mental map of how things connect, and decides what to do next.
...
- **Sonnet** is the default sweet spot for most coding.
- **Opus** is better when the hard part is reasoning (architecture, tricky debugging, high-stakes refactors).
- **Haiku** is the fastest and cheapest, but it’s more likely to miss nuance on complex codebases or multi-step tasks. 

Today’s state of the art is **Sonnet 4.5** and **Opus 4.5**, and these models keep improving over time.

For genuinely hard problems, you can also ask Claude to spend more on **thinking before answering** ([extended thinking](https://platform.claude.com/docs/en/build-with-claude/extended-thinking)). That usually means better plans and fewer dumb mistakes, but it increases **latency and cost**, so it’s best used selectively.

<aside>
At Rubric, we use Opus 4.5 as our daily driver. The unit cost is higher, but we usually get higher accuracy and fewer loops. We also tend to max out reasoning.
</aside>
...
If you look back at [the diagram above](#core-system-diagram), the loop is powered by one constraint: **whatever fits in the model’s context window is what exists for this step**.

Claude doesn’t have magical access to your whole project. It only sees what Claude Code has loaded into the window right now, and as the window fills up, older details get compacted.
...
<Collapsible title={<>What is <code>CLAUDE.md</code></>}>
    <code>CLAUDE.md</code> is a plain markdown file you add to your project to tell Claude Code “here’s how we do things here”. Unlike conversation history, which can be compacted, this is a set of fixed instructions always loaded into the context window.
</Collapsible>
...
Here’s the tool set Claude Code exposes, mapped to the gather/action/verify loop:
...
Beyond these core tools, Claude Code has a bunch of “niche” helpers that expand what it can do without changing the mental model. There are orchestration tools for delegation and tracking (`Task`, `TaskList`, `TaskOutput`), integrations via Model Context Protocol (MCP), notebook editing, and even language-server (LSP) code intelligence. You don’t use these every turn, but they’re what let Claude scale from “edit a file” to “run a workflow” when the task gets bigger.
...
Under the hood, the [text editor tool](https://platform.claude.com/docs/en/agents-and-tools/tool-use/text-editor-tool) is a great example of the design philosophy here: edits are **constrained**. Instead of “rewrite this file”, Claude is limited to a small command set (`view`, `create`, `insert`, `str_replace`), and the core edit primitive, `str_replace`, requires an **exact match** (including whitespace) so the change is surgically targeted.

That constraint matters because it fails loudly: if the old snippet doesn’t exist, or appears multiple times, your tool implementation should return an error, forcing Claude to go back and fetch more context until the edit is unambiguous. You can also cap what `view` returns with `max_characters` to keep large files from eating your context window.

A few high-signal takeaways that explain why this table matters.

First, `Bash` is what makes Claude Code feel “real”. It’s a persistent session, so Claude can run a sequence like “install deps → run tests → check diffs” and each step actually changes the same environment you’re working in.

Second, the web tools are powerful. `WebSearch` is great for freshness (and comes with citations), while `WebFetch` is for pulling in full pages, but even a normal doc page can add thousands of tokens and seconds of waiting.
...
That’s a useful mental model when you design your own tools. Let the large language model (LLM) generate the **intent** (which file, which command, which URL), and make your tools handle the **mechanics** (execution, guardrails, retries, truncation, and “fail loudly” behavior), so the agent spends tokens on reasoning instead of reinventing implementation details.
...
The mental model is simple: the main agent stays “in charge” of the narrative and the final decision, while sub-agents are disposable workers that return a summary you can keep.

### MCP

The Model Context Protocol (MCP) is a standard way to plug external systems into an agent as tools, like GitHub, Slack, databases, browsers, and internal APIs ([MCP](https://platform.claude.com/docs/en/agent-sdk/mcp)).
...
Claude’s approach is “load tools on demand”. In the Agent SDK, MCP tool search can automatically defer loading tool definitions and only expand the ones Claude actually needs, keeping the active context focused on the task ([MCP tool search](https://platform.claude.com/docs/en/agent-sdk/mcp)).

Finally, MCP fits the same safety story as the terminal: tools require explicit permission (for example via `allowedTools`), and you can scope what Claude is allowed to touch so the agent’s reach matches your comfort level.
...
- **Constrain tools on purpose**: let the large language model (LLM) choose the action, but let the tool enforce correctness and fail loudly.
- **Scale with isolation**: use sub-agents to keep context clean, and Model Context Protocol (MCP) when you need external systems without rewriting integrations.

If you want to go deeper, the best references are the [Agent SDK](https://platform.claude.com/docs/en/agent-sdk/overview) and the [tool use docs](https://platform.claude.com/docs/en/agents-and-tools/tool-use/overview).
