...
The system has four stages: fetch stats, generate a script, render frames, encode video.

### Fetching Stats

When a user authenticates with GitHub, we pull their year's activity via the GitHub GraphQL API: commits, pull requests, top languages, contributed repositories, stars given and received, and new followers. We request minimal permissions — no code access. Stats are stored in Supabase (Postgres + auth).
...
This was the stage that broke first. We launched, it went viral in the developer community, and the rendering pipeline buckled. We parallelized the Lambda architecture, added dynamic resolution downscaling for mobile, and built a queue system with per-user render-once caching (each user's download URL is stored in Supabase after first render).
...
Year in Code was used by thousands of developers. It demonstrated that LLMs could drive creative production at scale — not by generating media directly, but by making structured editorial decisions that feed deterministic rendering pipelines.

---

*Built in partnership with Graphite. Also published on [LangChain](https://blog.langchain.com/rubric-labs-graphite-personalized-video-at-scale/).*
