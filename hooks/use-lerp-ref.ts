...
export const useLerpRef = (
	...
	useEffect(() => {
		...
		const update = (time: number) => {
			...
			easing.damp(current, 'current', target.current, config.lerp, deltaTime / 1000)

			if (config.tickThreshold) {
				const diff = Math.abs(current.current - target.current)
				if (diff > config.tickThreshold) onTickRef.current?.(current.current)
			} else onTickRef.current?.(current.current)
