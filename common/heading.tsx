...
export function Heading({ tag, subtitle, className, align = 'center', ...props }: HeadingProps) {
	...
	return (
		<div className={$headingContainer({ align, className })}>
			...
			>
				<Comp
					className={clsx('text-pretty font-medium text-3xl md:text-4xl', {
			...
			{subtitle ? (
				<p
					className={clsx(
						'max-w-screen-md text-pretty font-light text-lg text-text-tertiary md:text-xl dark:text-dark-text-tertiary',
						{
							'text-center': align === 'center',
							'text-left': align === 'left',
							'text-right': align === 'right'
...
export function Tag({
	...
	return (
		<Comp
			className={clsx(
				'flex min-h-7 items-center justify-center gap-2 rounded-full bg-surface-secondary px-3.5 pb-px font-medium text-sm text-text-tertiary md:text-base dark:bg-dark-surface-secondary dark:text-dark-text-tertiary',
				className
			)}
			{...props}
		>
