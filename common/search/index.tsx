		...
		<SearchBox.Root search={search}>
			<Popover.Root
				...
				<Popover.Anchor asChild>
					...
					<label
						className={clsx(
							'border-border !ring-accent-500 dark:border-dark-border ml-auto flex w-full cursor-text items-center gap-x-1 rounded-full border px-3.5 py-2.5 focus-within:ring md:max-w-[280px]'
						)}>
						<SearchBox.Input
							asChild
							...
							<input
								className='placeholder:text-text-tertiary placeholder:dark:text-dark-text-tertiary grow bg-transparent !outline-none focus-visible:outline-none'
								placeholder='Search'
								type='text'
							/>
						</SearchBox.Input>
						...
						<div className='border-surface-tertiary bg-surface-primary dark:border-dark-surface-tertiary dark:bg-dark-surface-primary relative mx-5 min-h-20 w-[calc(100vw_-_2.5rem)] scroll-py-2 overflow-y-auto overscroll-y-contain rounded-xl border p-2 shadow-md md:mx-0 md:max-h-[320px] md:w-[550px]'>
							<SearchBox.Empty asChild>
								<div className='text-dark-text-tertiary absolute left-1/2 top-1/2 w-fit max-w-full -translate-x-1/2 -translate-y-1/2 items-center overflow-hidden text-ellipsis whitespace-nowrap px-2 py-1'>
									No results for{' '}
									<span className='font-medium'>&ldquo;{search.query}&rdquo;</span>
								</div>
							</SearchBox.Empty>
...
function HitBodyContainer({children}: React.PropsWithChildren) {
	return (
		<p className='text-text-tertiary dark:text-dark-text-tertiary truncate text-sm'>
			{children}
		</p>
	)
}
