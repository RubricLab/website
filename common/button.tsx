...
export const $button = cva(
	'gap-1 font-normal shrink-0 rounded-full ring-control focus-visible:ring-2 outline-none outline-0',
	{
		variants: {
			intent: {
				primary:
					'bg-accent-500 hover:bg-accent-600 text-textOnAccent-primary border-accent-600',
				secondary:
			...
			unstyled: {
				true:
					'px-0 py-0 bg-transparent border-none hover:bg-transparent hover:border-none dark:hover:bg-transparent dark:hover:border-none dark:bg-transparent dark:border-none'
			},
			size: {
...
type ButtonProps<C extends keyof JSX.IntrinsicElements> = VariantProps<
	typeof $button
> &
