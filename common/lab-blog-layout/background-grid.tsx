...
import { type Breakpoint, getCurrBreakpoint } from '@/lib/utils/breakpoints'
...
const BackgroundGrid = ({ data }: BackgroundGridProps) => {
	...
	useGSAP(
		() => {
			...
			for (const breakpoint of Object.keys(data)) {
				if (breakpoint !== bp) {
					gsap.set(`[data-grid-breakpoint="${breakpoint}"] [data-grid-col]`, {
						'--clip-progress': 1
					})
					gsap.set(`[data-grid-breakpoint="${breakpoint}"] [data-highlighted="true"]`, {
						'--background-opacity': 0.025
					})
				}
			}
									...
									style={
										{
											...
											'--clip-progress': 0,
											background: 'rgb(var(--color-surface-contrast) / var(--background-opacity))',
											clipPath:
												'polygon(0% 0%, 100% 0%, 100% calc(var(--clip-progress) * 100%), 0% calc(var(--clip-progress) * 100%))'
										} as CSSProperties
									}
