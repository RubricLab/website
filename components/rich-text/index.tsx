...
function Code({
	...
	if (isInline)
		return (
			<code className='border-border text-accent-500 dark:border-dark-border dark:bg-dark-surface-secondary rounded border px-2 py-0.5 before:[content:none] after:[content:none]'>
				{children}
			</code>
		)
	else
		return (
			<pre className='border-border dark:border-dark-border rounded-lg border'>
				{code}
			</pre>
		)
}
...
export function FaqRichtextComponent({
	...
	return (
		<details className='border-border bg-surface-secondary dark:border-dark-border dark:bg-dark-surface-secondary group mb-2 flex flex-col gap-4 overflow-hidden rounded-lg border pb-1'>
		...
		default:
			return (
				<article
					...
					id={_title}>
					<div className='text-accent-500 pr-2'>
						<svg
							fill='none'
							height='24'
							viewBox='0 0 24 24'
...
export function RichTextImage(props: {
	...
	return (
		<figure className='relative flex flex-col gap-2'>
			...
			/>
			{props.caption ? (
				<figcaption className='text-text-tertiary dark:text-dark-text-tertiary m-0 text-sm'>
					{props.caption}
				</figcaption>
			) : null}
		</figure>
