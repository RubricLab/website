...
export const richTextClasses = clsx(
	...
	'prose-h1:text-em-[36/16] prose-h1:font-medium prose-h1:text-text',
	'prose-h2:text-em-[32/16] prose-h2:font-semibold prose-h2:text-text',
	'prose-h3:text-em-[24/16] prose-h3:font-semibold prose-h3:text-text',
	'prose-h4:text-em-[20/16] prose-h4:font-medium prose-h4:text-text',
...
function Code({
	...
	if (isInline)
		return (
			<code className="rounded border border-border bg-surface-contrast/5 px-em-[8] py-em-[2] text-text-secondary before:[content:none] after:[content:none]">
				{children}
			</code>
		)
	return <pre className="border border-border">{code}</pre>
}
...
export function FaqRichtextComponent({ answer, _title }: FaqItemComponentRichText) {
	return (
		<details className="group mb-2 flex flex-col gap-4 overflow-hidden rounded-lg border border-border bg-surface-secondary pb-1 dark:border-dark-border dark:bg-dark-surface-secondary">
			<summary className="flex cursor-pointer items-start text-pretty rounded-md p-3 pb-2 pl-6 font-medium text-text-primary outline-none ring-accent-500 ring-inset focus-visible:ring dark:text-dark-text-primary">
...
const $richTextCallout = cva('gap-2 border border-border bg-surface-contrast/5 p-4 pl-3', {
	variants: {
		size: {
			large: 'flex flex-col',
